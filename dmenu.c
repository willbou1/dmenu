...
setup(void)
{
	...
	if (parentwin == root && (info = XineramaQueryScreens(dpy, &n))) {
		...
		if (centered) {
			mw = MIN(MAX(max_textw() + max_numbersw + promptw, min_width), info[i].width);
...
#endif
	{
		...
		if (centered) {
			mw = MIN(MAX(max_textw() + max_numbersw + promptw, min_width), wa.width);
			x = (wa.width  - mw) / 2;
			y = (wa.height - mh) / 2;
		} else {
			x = 0;
